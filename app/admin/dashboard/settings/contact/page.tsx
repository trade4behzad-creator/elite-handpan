  ...
  return (
    <div>
      <div style={{ marginBottom: '40px' }}>
        ...
        <h1 style={{ fontSize: '28px', fontWeight: '300', color: '#f5f5f5', margin: 0 }}>تماس با ما</h1>
      ...
      <div style={cardStyle}>
        <p style={{ fontSize: '12px', color: '#666', marginBottom: '24px' }}>
          این اطلاعات در صفحه تماس با ما و فوتر سایت نمایش داده می‌شود.
        </p>
        <SuccessBanner show={success === '1'} text="اطلاعات تماس ذخیره شد" />
        <ErrorBanner show={!!errorCode} text={errorCode === 'db' ? 'خطا در ذخیره تنظیمات' : ''} />
        <form action={updateContactSettings} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={fieldStyle}>
            <label style={labelStyle}>ایمیل</label>
            <input name="contact_email" type="email" required defaultValue={settings?.contact_email ?? ''} style={inputStyle} />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>تلفن (فرمت بین‌المللی، مثلا +989000000000)</label>
            <input name="contact_phone" required defaultValue={settings?.contact_phone ?? ''} style={inputStyle} />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>شماره واتساپ (فرمت بین‌المللی) — همینجا برای دریافت رسید سبد خرید هم استفاده می‌شود</label>
            <input name="whatsapp_number" required defaultValue={settings?.whatsapp_number ?? ''} style={inputStyle} />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>لینک اینستاگرام</label>
            <input name="instagram_url" required defaultValue={settings?.instagram_url ?? ''} style={inputStyle} />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>آدرس</label>
            <input name="contact_address" required defaultValue={settings?.contact_address ?? ''} style={{ ...inputStyle, direction: 'rtl' }} />
          </div>
          <button type="submit" style={saveButtonStyle}>ذخیره اطلاعات تماس</button>
        </form>
      </div>
    </div>
  )
